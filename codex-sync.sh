...
# 전역 규칙이 링크라 Claude 전용 줄을 뺄 수 없다. Codex 쪽에서 그 세 줄을 바로잡는다.
grep -q '^[^#]*developer_instructions' "$CODEX/config.toml" 2>/dev/null ||
  echo "codex-sync: config.toml 에 developer_instructions 가 없다. 교차 검증, doc-skill-guard, 독자 테스트 확인은 Claude 전용인데 전역 규칙에 그대로 남아 있다"
